...
import SettingsEditableField from './SettingsEditableField'
import { useState } from 'react'
import PropTypes from 'prop-types'
import { setFbUserInfo, selectFbPage } from '../../actions/facebook'
import { twitterLogin, twitterCodeVerify } from '../../actions/twitter'
...
const UserAuthFields = ({
	...
	const [twitterVerificationCode, setTwVerCode] = useState()

	// Retrieve current user's channel name from store
	const ytHandler = useSelector((state) => state.youtube.user.channelName) ?? ''
	const twRequestToken = useSelector(
		(state) => state.twitter.user.request_token
	)
	const twRequestTokenSecret = useSelector(
			...
			<div className='space-y-4'>
				...
				{twRequestToken && (
					<SettingsEditableField
						fieldName='Twitter Verification'
						inputTag='twitter-auth-handler'
						authenticate={() =>
							twitterCodeVerify(
								twitterVerificationCode,
								twRequestToken,
								twRequestTokenSecret
							)
						}
						updateState={setTwVerCode}
						icon={
							<svg
								className='w-4 h-4'
								role='img'
...
UserAuthFields.propTypes = {
	...
	twitterCodeVerify: PropTypes.func.isRequired,
}
