...
const ViewPost = () => {
	...
	useEffect(() => {
		...
		if (selected_post) {
			...
			let pMedia = selected_post.images
				? selected_post.images
				: selected_post.video

			let tempMedia = []
			pMedia.forEach((item) => {
				tempMedia.push(<img className={imgClassname} src={item.URL} />)
			})
			setMedia(tempMedia)
		} else {
		}
			...
			<div className='rounded shadow-lg bg-white w-1/2 max-h-3/4'>
				...
				<div className='px-5 py-2 space-y-2'>
					<div className='flex items-center space-x-2 mt-4'>
						...
						<div className='text-gray-800 leading-none capitalize text-xl font-bold'>
							{user.name}
						</div>
					</div>
